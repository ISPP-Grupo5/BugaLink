...
class Vehicle(models.Model):
    ...
    registration = models.CharField(max_length=256) # que es esto ¿model?
...
class DriverRoutine(models.Model):
    ...
    start_date = models.TimeField() # models.DateTimeField() en que quedamos
...
class IndividualLift(models.Model):
    ...
    status = models.CharField(max_length=256, choices=LiftStatus.choices(), default=LiftStatus.Pending_start)
    #acceptation_status y su enum?

class TravellerRoutine(models.Model):
    passenger = models.ForeignKey(Passenger, on_delete=models.CASCADE)
    start_date = models.TimeField() # en que quedamos en TimeField o dateTimeField?
    end_date = models.TimeField()
...
class Paypal(models.Model):
    mail = models.CharField(max_length=256, primary_key=True)   # Posible problema con multiples users, añadir id como en modelo
...
class FavDirection(models.Model):
    ...
    direction = models.CharField(max_length=256) # O esto
    city = models.CharField(max_length=256) # Puede llegar a ser redundante
    start_location = Coord(null=True)   # o esto
    cp = models.CharField(max_length=256) # Puede llegar a ser redundante/innecesario
